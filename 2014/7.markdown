    ...
    + 分支预测(Branch prediction)
        ...
        + 动态分支预测
            ...
            + BTB(Branch target buffer): 除了BPB的能力外，还能预测indirect jmp/call等的目标
                + Core i7有两个BTB
    + 高速缓存(Cache)
        + 一个现代CPU典型的参数是32K L1，256K L2，8M L3; 64字节的cache line; L1、L2是每个CPU核心独占，而L3是shared
            + 访问寄存器需要0.5~2 cycles, 而访问L1也只需要1~5 cycles
            + 访问主存需要50+ cycles
            + 访问SSD需要100us
            + 访问磁盘需要10ms
...
+ 虚拟存储器(Virtual memory)
    ...
    + 分页和MMU
        + TLB
        + 页表
            + Page entry table -> Page entry
        + 三级页表
            + Page directory table-> Page entry table -> Page entry
